...
class Db_Pdo extends Model_Db
{
    ...
    public function connect($config='', $linkNum=0)
    {
        if (!isset($this->linkID[$linkNum])) {
            ...
            try {
                ...
                $this->linkID[$linkNum] = new PDO($dsn, $config['username'], $config['password'], array(PDO::ATTR_PERSISTENT => false));
          ...
          //  $this->linkID[$linkNum]->exec('SET NAMES '.$config['charset']);
            ...
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if (1 != Model_Config::getDbGlobalConf('db_deploy_type')) {
                unset($this->config);
            }
        }
    ...
    public function execute($str, $bind=array())
    {
        ...
        if ($this->dbType == 'OCI') {
            if (preg_match("/^\s*(INSERT\s+INTO)\s+(\w+)\s+/i", $this->queryStr, $match)) {
                $this->table = Model_Config::getDbGlobalConf('db_sequence_prefix').str_ireplace(Model_Config::getDbGlobalConf('table_prefix'), "", $match[2]);
    ...
    protected function bindPdoParam($bind)
    {
        ...
        foreach ($bind as $key=>$val) {
            ...
            unset($val);
            $binding[1] = &$binding[1];
            call_user_func_array(array($this->PDOStatement,'bindParam'), $binding);
            unset($binding);
        }
    }
    ...
    {
        $this->initConnect(true);
        if (Model_Config::getDbGlobalConf('db_describe_table_sql')) {
            // 定义特殊的字段查询SQL
            $sql   = str_replace('%table%', $tableName, Model_Config::getDbGlobalConf('db_describe_table_sql'));
        } else {
            switch ($this->dbType) {
                case 'MSSQL':
                case 'SQLSRV':
    ...
    public function initDbType($config = array())
    {
        if (null === $this->dbType) {
            if (empty($config)) {
                $config =   $this->config;
    ...
    {
        if (Model_Config::getDbGlobalConf('db_fetch_table_sql')) {
            // 定义特殊的表查询SQL
            $sql   = str_replace('%db%', $dbName, Model_Config::getDbGlobalConf('db_fetch_table_sql'));
        } else {
            switch ($this->dbType) {
            case 'ORACLE':
            case 'OCI':
            ...
            case 'IBASE':
                // 暂时不支持
                throw new Exception('_NOT_SUPPORT_DB:IBASE');
                break;
            case 'SQLITE':
...
//    }
   ...
   /**
     * 插入记录 add by weixiaotong 2016-04-06
     * @access public
     * @param mixed $data 数据
     * @param array $options 参数表达式
     * @param boolean $replace 是否replace
                ...
                if (is_array($val) && 'exp' == $val[0]) {
                  ...
                  if ($index === 0) {
                    ...
                    if (Model_Config::getDbGlobalConf('db_bind_param') && 0 !== strpos($val, ':')) {
                        $name       =   md5($key . '_' . $index);
                        $arrValue[]   =   ':'.$name;
                        $this->bindParam($name, $val);
                    } else {
                        $arrValue[]   =  $this->parseValue($val);
                    }
                }
            }
    ...
    public function error()
    {
        ...
        WLog::warning('[ERROR SQL]'.$this->error, array());
        return $this->error;
    }
